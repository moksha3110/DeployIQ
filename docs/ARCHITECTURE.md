...
| **Loki + Winston**                                          | Structured app logs (Winston, JSON) shipped to Loki so logs and metrics live in the same Grafana pane — mirrors how logging is actually done in industry (correlate a metric spike with a log line). Shipped via a small hand-written transport, not the `winston-loki` npm package: that library's JSON batch encoder turned out to violate Loki's own push API schema (verified by reading its source and by the 400s Loki returned), so talking to Loki's push API directly — simple enough on its own — was the more reliable path. Promtail (bundled via the `loki-stack` chart) is a nice bonus on top: it scrapes every pod's container logs automatically, so _deployed user apps'_ logs show up in the same Grafana/Loki view with zero extra code. |
...
| **Vitest + Playwright**                                     | Unit coverage on services (build detection, manifest generation, webhook signature verification) via Vitest — the whole stack was already on it since Milestone 0, so it's used here instead of Jest to avoid a second test-runner config for one milestone. Playwright covers frontend routing/rendering against a mocked API (see Milestone 9 in `docs/MILESTONES.md` for why real GitHub OAuth isn't automated here).                                                                                                                                                                                                                                                                                                                                     |
| **React Flow (`@xyflow/react`)**                            | The M11.7 infrastructure topology graph needs an interactive, pannable/zoomable node-edge diagram with custom-styled nodes — reimplementing hit-testing and drag/pan/zoom by hand would be pure yak-shaving for a solved problem.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Recharts**                                                | M11.6's resource analytics dashboards need axis/tooltip/legend-aware line charts over time-series data with minimal boilerplate; the existing hand-rolled `Sparkline.tsx` (M5) intentionally stays simple (no axes/tooltips) for the compact metrics panel, so a real charting library was added rather than growing Sparkline into one.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **shadcn/ui (Radix + CVA, hand-written not CLI-generated)** | M11.10's dashboard overhaul needed accessible, composable primitives (Tabs, Button, Card, Badge) rather than more one-off Tailwind utility soup. shadcn/ui isn't an npm dependency — it's a convention (copy the component source into the repo, own it, no version lock-in) — so the components under `src/components/ui/` are hand-written to match that convention rather than fighting the CLI's interactive prompts non-interactively.                                                                                                                                                                                                                                                                                                                  |
| **PDFKit**                                                  | M11.9's downloadable reports need a real PDF, generated server-side and streamed directly to the response — PDFKit is a pure-JS, low-level PDF writer with no headless-browser dependency (unlike a Puppeteer HTML-to-PDF approach), which matters for a feature that's just formatted text/data, not a full page render.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
flowchart TB
    ...
    subgraph Platform["Backend Platform (Node/Express, containerized)"]
        ...
        INFRA["Infra Intelligence Service\n(health/security/cost score, incidents,\ntopology, NL query, PDF report)"]
    end
    ...
    INFRA -->|read live spec/pods/events| Cluster
    INFRA -->|PromQL query| PROM
    INFRA -->|structured + agentic tool-use| LLM
    INFRA --> PG
...
- **Infra Intelligence Service** (`modules/analysis/*`, M11) — reads live cluster state back from the Kubernetes API (something nothing before M11 did; the deploy pipeline only ever wrote manifests) and combines it with Prometheus actuals to compute a health score, security score, cost estimate, and incidents, each independently callable. A repeatable BullMQ job snapshots health/incidents every 5 minutes so trend views and incident detection work even with no dashboard open. The natural-language query endpoint is a small agentic loop over the same five functions as tools, so an AI-generated answer is never a separate, unverified code path from what the dashboard itself shows. The PDF report endpoint fans out to all of the above and streams a PDFKit document straight to the response.
